...
function setSensitiveConfig(key, value) {
  const sensitiveKeys = ['GEMINI_API_KEY', 'DOCUMENT_AI_PROCESSOR_ID', 'WEBHOOK_SECRET', 'MAIN_LEDGER_ID', 'GCP_PROJECT_ID'];
...
function setupGeminiApiKey(apiKey) {
  ...
  try {
    // 使用現有的 setSensitiveConfig 函數
    setSensitiveConfig('GEMINI_API_KEY', apiKey);
